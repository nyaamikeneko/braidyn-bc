...
- **HistoryなどのEMA回帰子をday頭で0にリセットすると、HMMの状態が1つその過渡に費やされる**: `src/glmhmm_ver4.py` の `add_history` は指数フィルタを各セッションの先頭で0から始める。押下率が0.765あるので定常値は`0.765/(1-0.65)=2.19`で、0起点だと`k=0`のz-score値が`-2.49`という極端な値になり、全dayで同じ形の立ち上がりが反復される。GLM-HMMに渡すと、この決定論的な過渡を説明するために状態が1つ割り当てられる（`VG1GC-66`で、1状態が90%以上を占めるdayが11/14→6/14に変わる）。回帰子を全dayプールの定常値から始めれば消える。詳細は[docs/findings_ver5.md](docs/findings_ver5.md) 5.1節。
- **Dynamic GLM-HMMは自前実装せず著者の公開実装を使う。遷移行列の事前分布は非対角にも擬似カウントを置くこと**: 同じ定式化・同じ尤度（3実装で差1e-13）でも、遷移行列のディリクレ事前分布の非対角を0にすると、期待遷移カウントが0の行がそのまま吸収状態になる。`VG1GC-66`ではこれで共有遷移行列に`[0, 1, 0]`の行ができ、14 dayすべてが同じ順序の3セグメントに切られて**状態がセッション内の時刻とほぼ一対一に対応する**解に落ちた（状態別のday内正規化位置が0.101±0.080など）。あわせて$\alpha$がグリッド上端に張り付き、重みが最大54.7まで発散する。著者実装（`priorDirP=[10,1]`で非対角に+1）では同じ入力でこれが起きず、$\alpha$も内点で選ばれる。切り替え回数や状態occupancyの指標だけでは退化を検出できないので、**状態別のday内位置とday別の状態系列パターンを必ず見る**。詳細は[docs/findings_ver5.md](docs/findings_ver5.md) 5.2・5.6節、対照実験は`notebooks/18_ver5_dynglmhmm_authors_impl.ipynb`。
