...
- **タスク**
  ...
  - `T_pull` は**成功率に応じて上下に動く適応的な閾値**（staircase）で、セッション内でもセッション間でも固定ではない（規則の詳細は下記「記録系」節）。
    - 調整は**片方向のラチェット**で、成功率が基準に達したときだけ閾値を上げ、セッション内で下げることはない（下がるのはセッション間の−100msのみ）。成功率を一定値に保つ制御ではない。
    - **成功率そのものは学習指標にならない**。動物が上達すると閾値が上がって難度で相殺されるため。論文が学習の指標として使うのはセッション終端の `T_pull`（`Tpull_final`）。
- **データセット構成**
...
- 音キュー: 10kHz純音、70dB SPL、200ms。キュー提示後1秒以内に規定時間 `T_pull` 以上レバーを**連続して**引き続けると成功、水報酬4μL。
- **キュー期間中に引けるのは1回だけ**。課題側がレバー位置をオンラインで監視しており、いったん pulled 状態に入った後 `T_pull` 経過前に non-pulled 状態へ戻ると、1秒の経過を待たずにその時点で **failure と確定して試行を中断する**。キュー期間中に一度も pulled 状態にならなかった場合が miss。
- **`T_pull` の適応規則（staircase）** — 論文Methods "Tone-triggered lever-pull task" より:
  - 初回セッションの初期値は 1ms。
  - **直近20試行の成功率が80%に達すると +50ms**。ただし上限は400ms。
  - 引き上げには**20試行の不応期**があり、各引き上げは最低20試行離れる。
  - **セッション内では決して短くならない**（単調非減少）。
  - セッション終端の値を `Tpull_final` と定義し、**次セッションの初期値は前セッションの `Tpull_final` − 100ms** に設定される。したがってセッション間では下がりうる。
  - 帰結: `T_pull` はセッション内で上がり、セッションの切り替わりで100ms下がる鋸歯状に動く。日をまたいで単調増加するわけではない。
- ITIは3–4秒のランダム。**ITI中にレバーを引くとITIがさらに3–4秒延長される**（引き続ける限り次の音キューが遅れる）。
- 論文の行動指標（Fig. 3）: response rate（valid pull数／音キュー数）、success rate（成功数／音キュー数）、`Tpull_final`。`Tpull_final` の時系列変化に基づく階層クラスタリング（Ward法）で個体が2クラスタ（A: n=7、B: n=18）に分かれる。
