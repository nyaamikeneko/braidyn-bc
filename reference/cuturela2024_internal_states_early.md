...
[Ashwood et al. (2022)](ashwood2022_discrete_strategies.md) の GLM-HMM は状態内のGLM重みと遷移行列をセッションを通じて固定のパラメータとして学習する（実質的に十分に訓練が進んだ定常状態のデータのみを対象とする）。本論文はこれを **"dynamic GLM-HMM"** として拡張し、遷移行列 $P^s$ と状態別GLM重み $w_k^s_k$ をセッション $s$ ごとに変化させる。
...
- **配列のインデックスはセッション単位ではなくデータ点単位**: `fit`に渡す重み `initW` は `N x K x D x C`、遷移行列 `initP` は `N x K x K` で、いずれも**全試行を連結した長さ N** で持つ（セッション数ではない）。`sessInd` に各セッションの開始インデックス（＋末尾+1）を渡してセッション境界を伝える。観測 `y` と欠測マスク `present` は1次元 `(N,)` を渡す（`(N,1)` にすると `backward_pass` が形状エラーになる）。参照クラスは **c=0**（`w[:,:,:,0]` を0に固定）で、`ssm` の「最後のクラスが参照」とは逆。
- **セッション数が20未満のときは初期状態分布の推定を拒否する**: `fit(..., fit_init_states=True)` は `if (fit_init_states==True and sess<20): raise Exception("Should not fit init states when less than 20 sessions due to high uncertainty")` で例外を投げる。既定は `False`（一様固定）。本リポジトリの対象は14 dayなので、この実装方針に従えば $\pi$ は推定しない（[docs/requirements_ver5.md](../docs/requirements_ver5.md) 5.2節）。
- **重みへの絶対的なL2罰則は**`'standard'`**にしか入らない**: `value_weight_loss_function`は`model_type == 'standard'`の分岐でのみ`L2penaltyW * -1/2 * w'w`を足す。`'partial'`/`'dynamic'`は前後セッションとの差分項（式1）だけで、しかもセッション0は`prevW=None`なので絶対的なアンカーを持たない。IBLの本番スクリプトでは`'standard'`が`L2penaltyW=1`、`'partial'`/`'dynamic'`が`L2penaltyW=0`。重みの大きさを直接抑えるものは動的段に存在せず、$\sigma$を大きくすると重みは無拘束になる。
- **遷移行列のディリクレ事前分布は非対角にも擬似カウントを置く**: `'standard'`/`'partial'`の閉形式更新は`p[i,j] = (zeta[i,j] + priorDirP[i!=j]) / (zeta[i,:] + priorDirP[i!=j] + priorDirP[i==j]*(K-1))`で、既定`priorDirP=[10,1]`は対角に10・**非対角に1**の擬似カウントを与える。非対角の擬似カウントを0にすると期待遷移カウントが0の行が厳密に0のままになり、吸収状態が生じる（[docs/findings_ver5.md](../docs/findings_ver5.md) 5.6節）。なおこの式は分母が$(i,j)$ごとに異なるため行方向に厳密には正規化されず、行和は期待遷移カウントが小さいほど1から外れる（本リポジトリの14 day・2359試行で0.998–0.9999）。
- `sigma`**は**`K x D`**配列で、状態・入力次元ごとに変えられる**: `fit`には`sigma[k,:]`として渡され、`invCov = diag(1/sigma^2)`になる。本番解析は`reshapeSigma`でスカラーをブロードキャストしているが、`analysis_utils.fit_eval_CV_2Dsigmas`（刺激列とそれ以外で別の$\sigma$）という2次元版も用意されている。
- **本番解析の$\sigma$グリッドと選択値**: `sigmaList = [10^-3, 10^-2.5, ..., 10^0.5] + [10, 10^2, 10^3]`（11点）で、`fit_cluster_dynamicGLMHMM.py`が使う`bestSigma`は`10^0.5 = 3.162`。上限$10^3$まで用意したグリッドの内点であり、論文本文の$\alpha\approx3$（集団平均3.2）と一致する。
- **本リポジトリの自前実装との一致を確認済み**: [notebooks/17_ver5_pre_implementation_checks.ipynb](../notebooks/17_ver5_pre_implementation_checks.ipynb) 6節の実装は、同一パラメータに対して本公開実装と同じ周辺対数尤度を返す（`VG1GC-66` の2359試行・14 day・$K=3$ で差 2.3e-13）。
