...
- **状態数の選択**: leave-one-session-out cross-validation（10通りの乱数初期値、動物ごとに個別）。1状態モデルに正規化したtest log-likelihoodの上昇量を指標に、1〜5状態を比較。マウスでは3状態（右バイアス・左バイアス・stochastic）が最も説明力が高く、以降の解析はこの3状態モデルに基づく（Extended Fig. 2の4戦略・4状態シミュレーションはパラメータ回復能力の検証専用で、実データにフィットする本番モデルではない）。
...
3. **デコード対象**: GLM-HMMのdiscrete state（$P(\text{state})\geq0.8$ で離散化、確率的 vs バイアス。左右バイアス状態は統合して単一の「バイアス」クラスにする）に加え、現在の報酬・空間選択・直前報酬・switch/stay選択・相手の選択も同じパイプラインでデコードし比較対象とする。全て二値分類として構成される（win/loss、L/R、switch/stay、stochastic/biasedなど）。空間選択のL/R情報は状態デコーディングとは独立に「Choice (L/R)」デコーディングとして別途扱う。
...
9. **直前報酬デコードの状態別比較（Figure 5c-e）**: 試行をGLM-HMM状態（バイアス vs 確率的、行動データのみに基づく分類）で分け、それぞれの状態内の試行群に限定して「直前試行の報酬あり/なし」を皮質活動から予測するSVMを独立に訓練・評価する。バイアス状態の試行群の方が確率的状態の試行群よりデコード精度が高く、確率的状態では過去の報酬に関する皮質表現が選択的に減弱していることを示す（H2支持）。
10. **報酬デコーディング軸への射影（Figure 5f-h, Figure 6e-g）**: 報酬（win/loss）デコーダのSVM重みベクトル（超平面の法線ベクトル）を「報酬デコーディング軸」とみなし、held-outデータへの内積射影により各試行の皮質活動を連続値スカラー（「どれだけ報酬寄りの活動パターンか」）に変換する。この射影値と次試行のswitch確率との相関を検証することで、報酬信号の強さが将来の選択行動をどれだけ予測するかを定量化する。
