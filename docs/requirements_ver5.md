...
* **観測モデル（2系統、切り替え可能）**:
  ...
  * **系統B（Categorical）**: 4値（Success / No Reaction / Short Pull / No Sound Pull）。多値emissionはHulsey et al. 2024の3値拡張を4値へ拡張する。
...
* 系統Aで $C=2$（No Reaction vs それ以外）とした多項ロジットは、パラメータ化こそ違うがBernoulliと同じ分布族になる。つまり系統Bは系統Aの厳密な一般化で、$C$ を切り替える実装として両者を1本のコードで扱える。
...
Step 1–3は両系統で共通（1本の試行テーブルを作る）。Step 4–6を系統A・Bそれぞれで走らせ、Step 7で比較してから、Step 8以降を採用系統で進める。
