...
Ver.5 の設計が前提としている事柄を実装前に測った結果は [findings_ver5.md](findings_ver5.md) にあります。要件定義には決定と1行の根拠だけを置き、個体・実行時点に依存する数値（day別の内訳、報酬閾値の推移、cross-validationの尤度表、状態構造の指標など）はこちらに集約しています。生成元はノート `17` と `18` です。
...
主な結果は4つ。(1) 反応率・成功率・報酬閾値 `Tpull_final` のいずれもday indexに単調傾向を持たず、`Tpull_final` はday5–7で上限0.40秒に達したあと後退する（`VG1GC-66` では Day 1–15 が単調な学習過程になっていない）。day6は日レベルの外れ値ではなくセッション内の崩壊（日内3分割のNo Reaction率 0.16 → 0.74 → 0.93）。(2) Short Pullは接触途切れ62.0% / 引き足りない36.0% / 締め切り超過1.8%に分かれ、系列内の時間的まとまり・3次元入力からの予測可能性はいずれも `yA` の1桁下（接触途切れ型を除いても回復しない）。(3) $P(z)\geq0.8$ の残存率は84.4%だがday9は12.5%、day内で状態×試行タイプを釣り合わせると14 dayすべてで使用可能試行が0件になる。原因は静的フィットの状態がdayラベルに退化していること（1状態が90%以上を占めるdayが11/14、day内の状態切り替えは全期間で3回）。(4) `ssm` の `InputDrivenObservations(C=3)` で系統Bの静的フィットが学習でき、$C=2$ に潰すと系統Aと log probability が一致する。

6節では [requirements_ver5.md](requirements_ver5.md) 5.2〜5.4節の Dynamic GLM-HMM（Cuturela et al. 2024）を自前で実装する。`model_type ∈ {'standard', 'partial', 'dynamic'}` の3段階、スケーリング付き前向き後向き、重みのMステップ（全day同時のL-BFGS-B）、遷移行列の閉形式MAP、ブロック分割CVと欠測マスクでの評価を持つ。合成データでのパラメータ復元、`ssm` および著者の公開実装との対数尤度一致の検証、$\alpha$／$\kappa$／$K$ のグリッドサーチ、日内の状態切り替えの判定、動的フィットの状態での皮質デコード試行数の再評価を行う。

**この6節の自前実装が返す解は退化している**。共有遷移行列が吸収的になり、14 dayすべてが同じ順序の3セグメントに切られて、状態が day 内の時刻とほぼ一対一に対応する。$\alpha$ もグリッド上端に張り付き、重みは最大54.7まで発散する。同じ定式化を著者の公開実装で解くとこれらは起きない（ノート `18`）ので、6節の $\alpha$／$\kappa$／$K$ の選択結果と、そこから出た「day内バランシング後372試行」という見積もりは、**この実装に固有の数値として読む**。診断と対照実験はノート `18`、実測値は [findings_ver5.md](findings_ver5.md) 5節にある。

グリッドサーチとNWB読み込みは `data/cache/`（`.gitignore` 済み）にキャッシュされ、キャッシュが無ければノートが自前で計算する。

### 18_ver5_dynglmhmm_authors_impl.ipynb

ノート `17` 6節の状態退化の原因を切り分けるため、Cuturela et al. 2024 の公開実装 [lenca56/dynamic_glmhmm](https://github.com/lenca56/dynamic_glmhmm) を直接使って同じデータを学習し直すノート。公開実装は `external/dynamic_glmhmm`（`.gitignore` 済み）に置き、無ければノートがクローンする。$K=3$・系統A固定で、$K$ と $\kappa$ の掃引は行わない。

2つの修正を入れ、それぞれの効果を対照実験で切り分ける。

- **History 回帰子の day 頭初期値**: 指数フィルタを0からではなく全dayプールの定常値から始める。0起点だと day 冒頭に決定論的な過渡が生じ、z-score後の $k=0$ が $-2.5$ 前後になる。
- **重みの変動幅 $\sigma$（論文の $\alpha_{k,d}$）**: 論文と同じ $10^{-3}$–$10^{3}$ グリッドでCVする。列別 $\sigma$（Bias列 vs History 2列）はBias座標のみ掃引して打ち切っている。

$\sigma$ のCVは**修正前後の両方の入力**に対して行う。系列・観測・fold分割は共通で入力の day 冒頭だけが違うので、$\sigma$ の挙動の変化がどちらに由来するかが直接読める。

主な結果は4つ。(1) $\sigma$ は入力の作り方によらずグリッド内点（$\sigma=1$）で最良になる。ノート `17` の自前実装がグリッド上端に張り付いたのは入力ではなく実装に由来する。(2) 遷移行列が吸収的でなくなり、day 内の時刻への退化が解消する（$\max|W|$ は54.7から2.7へ、状態別の day 内位置は0.49–0.66・sd 0.28–0.32）。(3) History の初期値を直すと、1状態が90%以上を占める day が11/14から6/14に減る。(4) 一方で $P(z)\geq0.8$ の残存率は0.351まで下がり、day内バランシング後に使える試行は全day計36件（day12のみ）、13/14 dayで0件になる。ノート `17` の372件は退化した状態から出た数字だった。

図は、day冒頭の入力の立ち上がり、$\sigma$ のCV曲線（修正前後の2本）、**dayごとのパネル図**、day別のGLM重み軌跡（状態×入力の9パネル）、状態確信度のヒストグラムとday別使用可能試行数。dayごとのパネル図はノート `16`（`v4.plot_day_panel`）と同じ構成で、試行インデックスを共有軸にtrial type・action history・reward history・state posterior・MAP状態を1枚に重ねる（Ver.5 は入力からStimulusを落としているのでStimulus段は無く、最下段はViterbi系列ではなく試行ごとの周辺事後確率のargmax）。主対象の fixed/partial について14 day分を個別に描き、比較用の reset/partial と fixed/standard は全dayを1枚に収めた積み上げ図で並べる。

フィット結果は `data/cache/ver5_note18.pkl`（`.gitignore` 済み）にキャッシュされる。キャッシュが無い場合の全計算は約2時間かかる。
