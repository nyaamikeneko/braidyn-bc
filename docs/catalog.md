...
`docs/` と `notebooks/` の対応表です。セットアップは [README.md](../README.md) を見てください。
...
| :--- | :--- | :--- |
| 立場 | ノート `10` / `11` / `12` が実装している仕様 | 次に移す仕様。ノート未実装 |
...
- `config.py` — Colab / ローカルで NWB・CSV のルートを切り替える。
