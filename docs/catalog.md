...
Ver.5 の設計が前提としている事柄を実装前に測った結果は [findings_ver5.md](findings_ver5.md) にあります。要件定義には決定と1行の根拠だけを置き、個体・実行時点に依存する数値（day別の内訳、報酬閾値の推移、cross-validationの尤度表、状態構造の指標など）はこちらに集約しています。生成元はノート `17` です。
...
6節では [requirements_ver5.md](requirements_ver5.md) 5.2〜5.4節の Dynamic GLM-HMM（Cuturela et al. 2024）を実装する。`model_type ∈ {'standard', 'partial', 'dynamic'}` の3段階、スケーリング付き前向き後向き、重みのMステップ（全day同時のL-BFGS-B）、遷移行列の閉形式MAP、ブロック分割CVと欠測マスクでの評価を持つ。合成データでのパラメータ復元、`ssm` および著者の公開実装との対数尤度一致の検証、$\alpha$／$\kappa$／$K$ のグリッドサーチ、日内の状態切り替えの判定（8節Step 6の関門）、動的フィットの状態での皮質デコード試行数の再評価を行う。主な結果は、day別の重みは効くが「緩やかな変化」の前提とday別遷移行列は支持されないこと、動的化で状態がdayラベルから脱してday内バランシング後の使用可能試行が0→372試行になること。
