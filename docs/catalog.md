...
図は、day冒頭の入力の立ち上がり、$\sigma$ のCV曲線（修正前後の2本）、**14 day分の事後状態確率のパネル図**（fixed/partial・reset/partial・fixed/standard の3通り）、day別のGLM重み軌跡（状態×入力の9パネル）、状態確信度のヒストグラムとday別使用可能試行数。
