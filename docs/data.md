...
- **成功率は学習曲線として使えない。** 調整は片方向のラチェットで、動物が上達すると閾値が上がって難度で相殺されるため、成功率の日変化は能力の変化を反映しない。ただし**成功率が80%に保たれるわけではない**: `VG1GC-66` の実測では day別成功率（全音提示試行）の中央値は 0.566（範囲 0.280–0.883、全体 0.548）で、20試行移動窓の成功率が0.8以上だった割合は全day平均で13.7%にとどまる（day8・day9・day15 は0%）。閾値の引き上げは全14日で計23回（day中央値1回）。成功率（全試行）のday index相関は Spearman −0.354（p=.215）、押下試行内の成功率は −0.670（p=.009）。論文自身も学習の指標には成功率ではなく `Tpull_final` を使い、その時系列で個体を2クラスタに分けている。
